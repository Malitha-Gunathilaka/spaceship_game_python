...
# Load images
spaceship_image = pygame.image.load(os.path.join(base_dir, "image", "spaceship.png")).convert_alpha()
asteroid_image = pygame.image.load(os.path.join(base_dir, "image", "asteroid.png")).convert_alpha()
bullet_image = pygame.image.load(os.path.join(base_dir, "image", "bullet.png")).convert_alpha()
power_up_image = pygame.image.load(os.path.join(base_dir, "image", "power_up.png")).convert_alpha()
background_image = pygame.image.load(os.path.join(base_dir, "image", "background.png")).convert_alpha()
pause_button_image = pygame.image.load(os.path.join(base_dir, "image", "pause.png")).convert_alpha()
play_button_image = pygame.image.load(os.path.join(base_dir, "image", "play.png")).convert_alpha()
sound_on_image = pygame.image.load(os.path.join(base_dir, "image", "sound_on.png")).convert_alpha()
sound_off_image = pygame.image.load(os.path.join(base_dir, "image", "sound_off.png")).convert_alpha()
start_button_image = pygame.image.load(os.path.join(base_dir, "image", "start_button.png")).convert_alpha()
...
hit_sound = pygame.mixer.Sound(os.path.join(base_dir, "sound", "hit.wav"))
lose_life_sound = pygame.mixer.Sound(os.path.join(base_dir, "sound", "lose_life.wav"))
power_up_sound = pygame.mixer.Sound(os.path.join(base_dir, "sound", "power_up.wav"))
...
def reset_game():
    global player_pos, bullets, asteroids, power_ups, score, lives, level, asteroid_speed, paused
    player_pos = [screen_width // 2, screen_height - player_size - 10]
    bullets = []
    asteroids = []
    power_ups = []
...
def draw_home_screen():
    ...
    draw_text("Spaceship Shooting Game", (255, 255, 255), screen_width // 2 - 150, screen_height // 2 - 100)
...
while running:
    if not game_started:
        ...
        if not paused:
            ...
            asteroids = [asteroid for asteroid in asteroids if asteroid[1] < screen_height]
            ...
            for bullet in bullets:
                for asteroid in asteroids:
                    if (bullet[0] < asteroid[0] + asteroid[2] and bullet[0] + bullet_size > asteroid[0] and
                            bullet[1] < asteroid[1] + asteroid[2] and bullet[1] + bullet_size > asteroid[1]):
            ...
            for asteroid in asteroids:
                if (player_pos[0] < asteroid[0] + asteroid[2] and player_pos[0] + player_size > asteroid[0] and
                        player_pos[1] < asteroid[1] + asteroid[2] and player_pos[1] + player_size > asteroid[1]):
                    asteroids.remove(asteroid)
                    lives -= 1
            ...
            for power_up in power_ups:
                if (player_pos[0] < power_up[0] + power_up_size and player_pos[0] + player_size > power_up[0] and
                        player_pos[1] < power_up[1] + power_up_size and player_pos[1] + player_size > power_up[1]):
        ...
        # Draw player, bullets, asteroids, and power-ups
        ...
        draw_text(f"Lives: {lives}", (255, 255, 255), screen_width - 110, 10)
        draw_text(f"Level: {level}", (255, 255, 255), screen_width // 2 - 40, 10)

        # Draw pause/play button
        if paused:
            screen.blit(play_button_image, (screen_width - button_size - 10, 50))
        else:
            screen.blit(pause_button_image, (screen_width - button_size - 10, 50))

        # Draw sound button
        if music_muted:
            screen.blit(sound_off_image, (screen_width - button_size - 10, 110))
        else:
            screen.blit(sound_on_image, (screen_width - button_size - 10, 110))

        pygame.display.update()
        clock.tick(30)
    else:
        draw_text("Game Over", (255, 0, 0), screen_width // 2 - 70, screen_height // 2 - 50)
